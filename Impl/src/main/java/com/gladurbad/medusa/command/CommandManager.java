...
public final class CommandManager implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String string, String[] args) {
        if (commandSender.hasPermission("skidusa.commands")) {
            if (args.length > 0) {
                for (final MedusaCommand medusaCommand : commands) {
                    ...
                    if (commandName.equals(args[0])) {
                        if (commandSender.hasPermission("skidusa." + commandName)) {
                            if (!medusaCommand.handle(commandSender, command, string, args)) {
                                commandSender.sendMessage(ColorUtil.translate(Config.ACCENT_ONE + "Usage: /skidusa " +
                ...
                commandSender.sendMessage(ColorUtil.translate(Config.ACCENT_ONE + "Skidusa AntiCheat Commands:\n" + " \n"));
                for (final MedusaCommand medusaCommand : commands) {
                    commandSender.sendMessage(ColorUtil.translate( Config.ACCENT_ONE + "/skidusa " +
                            medusaCommand.getCommandInfo().name() + " " +
                            medusaCommand.getCommandInfo().syntax()));
                }
                commandSender.sendMessage("");
