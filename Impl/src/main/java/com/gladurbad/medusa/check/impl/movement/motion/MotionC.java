    ...
    public void handle(final Packet packet) {
        if (packet.isPosition()) {
            ...
            if (data.getVelocityProcessor().getTicksSinceVelocity() <= 5) {
                maxHighJump = 0.53F + (double) ((float) PlayerUtil.getPotionLevel(data.getPlayer(), PotionEffectType.JUMP) * 0.1F) +
                ...
                maxHighJump = 0.43F + (double) ((float) PlayerUtil.getPotionLevel(data.getPlayer(), PotionEffectType.JUMP) * 0.1F) +
                        (data.getVelocityProcessor().getTicksSinceVelocity() < 5 ? data.getVelocityProcessor().getVelocityY() > 0 ? data.getVelocityProcessor().getVelocityY() : 0 : 0);
            }
            ...
            final boolean exempt = isExempt(
                    ExemptType.INSIDE_VEHICLE, ExemptType.FLYING, ExemptType.UNDER_BLOCK,
                    ExemptType.PISTON, ExemptType.LIQUID, ExemptType.NEAR_VEHICLE, ExemptType.TELEPORT,
                    ExemptType.WEB, ExemptType.PLACING, ExemptType.SLIME, ExemptType.CLIMBABLE//, ExemptType.STAIRS
            );

            debug("deltaY" + deltaY + " ejm=" + expectedJumpMotion + " mhj=" + maxHighJump + " step=" + step);
            //Mini-jump check.
            if (jumped && !exempt && data.getVelocityProcessor().getTicksSinceVelocity() > 4) {
            ...
            //Generic step check.
            if (step && !isExempt(ExemptType.TELEPORT) && !data.getPositionProcessor().isOnSlime()) {
                if (deltaY > 0.6F) {
