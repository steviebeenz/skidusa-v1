...
public class JesusB extends Check {
    ...
    public void handle(Packet packet) {
        if (packet.isPosition()) {
            ...
            final boolean touchingWater = data.getPositionProcessor().isInWater();
            final boolean nearGround = data.getPositionProcessor().isOnSolidGround();

            final boolean exempt = isExempt(ExemptType.VELOCITY, ExemptType.FLYING,
                    ExemptType.DEPTH_STRIDER, ExemptType.CARPET, ExemptType.LILYPAD,
            ...
            if (!exempt && touchingWater && (invalidAccel || invalidDelta)) {
                if (++buffer > buffers) fail("dY= " + deltaY + " diff= " + diff);
            } else {
