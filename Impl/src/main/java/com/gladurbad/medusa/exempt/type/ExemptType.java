...
public enum ExemptType {
    ...
    RESPAWN(data -> data.getActionProcessor().isRespawning()),

    JOINED(data -> System.currentTimeMillis() - data.getJoinTime() < 5000L),

    SOULSAND(data -> data.getPositionProcessor().isOnSoulSand()),

    GROUND(data -> data.getPositionProcessor().isOnGround()),
    ...
    LAGGING(data -> data.getActionProcessor().isLagging()),

    CARPET(data -> data.getPositionProcessor().isNearCarpet()),

    LILYPAD(data -> data.getPositionProcessor().isNearLilyPad()),
    ...
    WATER(data -> data.getPositionProcessor().isInWater()),
